...
function makeURL(module, action) {
    if (Config.JQ_DEBUG) {
        return `http://${Config.DEBUG_DOMAIN}/onetypecho/v1/${module}/${action}`;
    } else {
        return `https://${Config.JQ_DOMAIN}/onetypecho/v1/${module}/${action}`;
    }
}

function makeRes(res, name) {
    if (Config.JQ_DEBUG) {
        return `http://${Config.DEBUG_DOMAIN}/usr/plugins/OneTypecho/assets/${res}/${name}`;
    }else{
        return `https://${Config.JQ_DOMAIN}/usr/plugins/OneTypecho/assets/${res}/${name}`;
    }
}
