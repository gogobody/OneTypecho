...
const helper = {
  ...
  _downloadImageAndInfo(image, index, pixelRatio) {
    return new Promise((resolve, reject) => {
      ...
      const imageUrl = url; // 下载图片
      this._downImage(imageUrl, index) // 获取图片信息
      .then(imgPath => this._getImageInfo(imgPath, index)).then(({
        imgPath,
        imgInfo
      }) => {
  ...
  _downImage(imageUrl) {
    return new Promise((resolve, reject) => {
	  ...
	  let dataPath = wx.env.USER_DATA_PATH;
	  // #endif
	  // #ifdef MP-QQ
	  let dataPath = qq.env.USER_DATA_PATH;
	  // #endif
	  // #ifdef APP-PLUS
	  let dataPath = `_doc/parser_tmp`;
	  // #endif
      if (/^http/.test(imageUrl) && !new RegExp(dataPath).test(imageUrl)) {
		  uni.downloadFile({
...
export default {
  ...
  methods: Object.assign({
          ...
          create(config) {
			  ...
			  // #endif
			  // #ifdef MP-QQ
			  this.ctx = qq.createCanvasContext('canvasid', this);
			  // #endif
                          ...
                          setTimeout(() => {
							  ...
							  // #endif
                                  ...
                                  success: (res) => {
                                      ...
                                      that.$emit('success', res.tempFilePath);//triggerEvent
                                  },
                                  fail: (err) => {
                                      that.$emit('fail', err);
                                  },
